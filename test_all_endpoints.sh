#!/bin/bash
# ════════════════════════════════════════════════════════════════════
# Zeen — Full API Test Script
# Tests all 35 functional requirements against localhost:8000
#
# Usage:
#   chmod +x test_all_endpoints.sh
#   ./test_all_endpoints.sh
#
# Prerequisites: docker-compose up --build  (all services running)
# ════════════════════════════════════════════════════════════════════

BASE="http://localhost:8000/api/v1"
PASS=0
FAIL=0
TOTAL=0

# -detailed → print request + response bodies. Default: status lines only.
DETAILED=0
for arg in "$@"; do
    case "$arg" in
        -detailed|--detailed) DETAILED=1 ;;
    esac
done

green()  { printf "\033[32m✓ %s\033[0m\n" "$1"; }
red()    { printf "\033[31m✗ %s\033[0m\n" "$1"; }
header() { printf "\n\033[1;34m── %s ──\033[0m\n" "$1"; }

# show_request <method> <url> <body> — dimmed preview of an outgoing request.
# Writes to stderr so it survives $(do_curl …) command substitution.
show_request() {
    [ "$DETAILED" = "1" ] || return 0
    local method="$1" url="$2" body="$3"
    local short_url="${url#$BASE}"
    [ "$short_url" = "$url" ] || short_url="…${short_url}"
    printf "  \033[36m→ %s %s\033[0m\n" "$method" "$short_url" >&2
    [ -z "$body" ] && return
    python3 - "$body" >&2 2>/dev/null <<'PY' || printf "  \033[2m  body: %s\033[0m\n" "${body:0:160}" >&2
import json, sys
DIM, RESET = "\033[2m", "\033[0m"
raw = sys.argv[1]
try:
    data = json.loads(raw)
except Exception:
    snippet = raw.strip().replace("\n", " ")[:160]
    print(f"  {DIM}  body: {snippet}{RESET}")
    sys.exit(0)
text = json.dumps(data, indent=2, default=str, ensure_ascii=False)
lines = text.splitlines()
for line in lines[:10]:
    print(f"  {DIM}  {line}{RESET}")
if len(lines) > 10:
    print(f"  {DIM}  …{RESET}")
PY
}

# do_curl ARGS… — like curl, but prints a request preview first.
# Parses -X / -d / URL out of the arg list to build the preview.
do_curl() {
    local method="GET" url="" body=""
    local args=("$@")
    local i=0 n=${#args[@]}
    while [ $i -lt $n ]; do
        case "${args[$i]}" in
            -X|--request)            i=$((i+1)); method="${args[$i]}" ;;
            -d|--data|--data-raw|--data-binary)
                                     i=$((i+1)); body="${args[$i]}" ;;
            http://*|https://*)      url="${args[$i]}" ;;
        esac
        i=$((i+1))
    done
    [ -n "$body" ] && [ "$method" = "GET" ] && method="POST"
    show_request "$method" "$url" "$body"
    curl "$@"
}

# pretty <file> — print a compact, dimmed preview of a JSON response.
pretty() {
    [ "$DETAILED" = "1" ] || return 0
    local f="${1:-/tmp/z_last.json}"
    [ ! -s "$f" ] && { printf "  \033[2m└─ (empty body)\033[0m\n"; return; }
    python3 - "$f" <<'PY' 2>/dev/null || printf "  \033[2m└─ (non-JSON, %d bytes)\033[0m\n" "$(wc -c < "$f")"
import json, sys
DIM, RESET = "\033[2m", "\033[0m"
PREFIX = f"  {DIM}│ "
with open(sys.argv[1]) as fp:
    raw = fp.read()
try:
    data = json.loads(raw)
except Exception:
    snippet = raw.strip().replace("\n", " ")[:160]
    print(f"  {DIM}└─ {snippet}{RESET}")
    sys.exit(0)

def emit(text, extra=""):
    lines = text.splitlines()
    for line in lines[:8]:
        print(f"{PREFIX}{line}{RESET}")
    if len(lines) > 8:
        print(f"{PREFIX}…{RESET}")
    if extra:
        print(f"  {DIM}└─ {extra}{RESET}")
    else:
        print(f"  {DIM}└─{RESET}")

if isinstance(data, list):
    total = len(data)
    preview = data[:2]
    text = json.dumps(preview, indent=2, default=str, ensure_ascii=False)
    extra = f"array · {total} item(s)" + (f" · showing first 2" if total > 2 else "")
    emit(text, extra)
elif isinstance(data, dict):
    text = json.dumps(data, indent=2, default=str, ensure_ascii=False)
    emit(text, f"object · {len(data)} field(s)")
else:
    print(f"  {DIM}└─ {data}{RESET}")
PY
}

check() {
    TOTAL=$((TOTAL+1))
    local label="$1" code="$2" expected="$3" body="${4:-/tmp/z_last.json}"
    if [ "$code" = "$expected" ]; then
        green "$label (HTTP $code)"
        PASS=$((PASS+1))
    else
        red "$label (expected $expected, got $code)"
        FAIL=$((FAIL+1))
    fi
    pretty "$body"
}

# ── FR-01: User Registration ───────────────────────────────────────
header "FR-01 User Registration"

# Register admin
R=$(do_curl -s -o /tmp/z_admin.json -w "%{http_code}" -X POST "$BASE/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@zeenapp.com","username":"admin_test","password":"admin123","first_name":"Admin","last_name":"User","role":"admin"}')
check "Register admin user" "$R" "200" /tmp/z_admin.json

# Register normal user
R=$(do_curl -s -o /tmp/z_user.json -w "%{http_code}" -X POST "$BASE/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"email":"user@zeenapp.com","username":"normal_user","password":"user1234","first_name":"Normal","last_name":"User"}')
check "Register normal user" "$R" "200" /tmp/z_user.json

# Register second user (for testing)
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"email":"user2@zeenapp.com","username":"second_user","password":"user1234","first_name":"Second","last_name":"User"}')
check "Register second user" "$R" "200"

# Duplicate check
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@zeenapp.com","username":"admin_test","password":"admin123"}')
check "Reject duplicate registration" "$R" "400"

# ── FR-02: User Login ──────────────────────────────────────────────
header "FR-02 User Login"

R=$(do_curl -s -o /tmp/z_admin_token.json -w "%{http_code}" -X POST "$BASE/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@zeenapp.com","password":"admin123"}')
check "Admin login" "$R" "200" /tmp/z_admin_token.json
ADMIN_TOKEN=$(cat /tmp/z_admin_token.json | python3 -c "import sys,json; print(json.load(sys.stdin)['access_token'])" 2>/dev/null)

R=$(do_curl -s -o /tmp/z_user_token.json -w "%{http_code}" -X POST "$BASE/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"user@zeenapp.com","password":"user1234"}')
check "Normal user login" "$R" "200" /tmp/z_user_token.json
USER_TOKEN=$(cat /tmp/z_user_token.json | python3 -c "import sys,json; print(json.load(sys.stdin)['access_token'])" 2>/dev/null)

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@zeenapp.com","password":"wrongpass"}')
check "Reject bad password" "$R" "401"

AUTH_ADMIN="Authorization: Bearer $ADMIN_TOKEN"
AUTH_USER="Authorization: Bearer $USER_TOKEN"

# ── FR-16: Token Validation ────────────────────────────────────────
header "FR-16 Token Validation"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/auth/validate-token" -H "$AUTH_USER")
check "Validate token" "$R" "200"

# ── FR-03: User Profile Management ────────────────────────────────
header "FR-03 User Profile Management"

R=$(do_curl -s -o /tmp/z_me.json -w "%{http_code}" "$BASE/auth/me" -H "$AUTH_USER")
check "Get current user (me)" "$R" "200" /tmp/z_me.json
USER_ID=$(cat /tmp/z_me.json | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null)
ADMIN_ID=$(curl -s "$BASE/auth/me" -H "$AUTH_ADMIN" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null)

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X PUT "$BASE/users/$USER_ID" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"first_name":"Updated"}')
check "Update own profile" "$R" "200"

# ── FR-17: Change Password ────────────────────────────────────────
header "FR-17 Change Password"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/auth/change-password" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"current_password":"user1234","new_password":"newpass123"}')
check "Change password" "$R" "200"

# Re-login with new password
R=$(do_curl -s -o /tmp/z_user_token2.json -w "%{http_code}" -X POST "$BASE/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"user@zeenapp.com","password":"newpass123"}')
check "Login with new password" "$R" "200" /tmp/z_user_token2.json
USER_TOKEN=$(cat /tmp/z_user_token2.json | python3 -c "import sys,json; print(json.load(sys.stdin)['access_token'])" 2>/dev/null)
AUTH_USER="Authorization: Bearer $USER_TOKEN"

# ── FR-13: Content Ingestion (Admin) ──────────────────────────────
header "FR-13 Content Ingestion (Admin)"

R=$(do_curl -s -o /tmp/z_movie1.json -w "%{http_code}" -X POST "$BASE/content/" \
  -H "$AUTH_ADMIN" -H "Content-Type: application/json" \
  -d '{
    "title":"Interstellar","description":"A team of explorers travel through a wormhole in space","content_type":"movie",
    "duration_minutes":169,"release_date":"2014-11-07T00:00:00","rating":0,
    "genres":["Sci-Fi","Drama","Adventure"],"cast":["Matthew McConaughey","Anne Hathaway","Jessica Chastain"],
    "directors":["Christopher Nolan"],"poster_url":"https://example.com/interstellar.jpg"
  }')
check "Admin creates movie (Interstellar)" "$R" "200" /tmp/z_movie1.json
MOVIE1_ID=$(cat /tmp/z_movie1.json | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null)

R=$(do_curl -s -o /tmp/z_movie2.json -w "%{http_code}" -X POST "$BASE/content/" \
  -H "$AUTH_ADMIN" -H "Content-Type: application/json" \
  -d '{
    "title":"The Dark Knight","description":"Batman faces the Joker, a criminal mastermind","content_type":"movie",
    "duration_minutes":152,"release_date":"2008-07-18T00:00:00","rating":0,
    "genres":["Action","Crime","Drama"],"cast":["Christian Bale","Heath Ledger","Aaron Eckhart"],
    "directors":["Christopher Nolan"]
  }')
check "Admin creates movie (Dark Knight)" "$R" "200" /tmp/z_movie2.json
MOVIE2_ID=$(cat /tmp/z_movie2.json | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null)

R=$(do_curl -s -o /tmp/z_series1.json -w "%{http_code}" -X POST "$BASE/content/" \
  -H "$AUTH_ADMIN" -H "Content-Type: application/json" \
  -d '{
    "title":"Breaking Bad","description":"A high school chemistry teacher turned drug kingpin","content_type":"series",
    "duration_minutes":0,"release_date":"2008-01-20T00:00:00","rating":0,
    "genres":["Crime","Drama","Thriller"],"cast":["Bryan Cranston","Aaron Paul","Anna Gunn"],
    "directors":["Vince Gilligan"]
  }')
check "Admin creates series (Breaking Bad)" "$R" "200" /tmp/z_series1.json
SERIES1_ID=$(cat /tmp/z_series1.json | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null)

# Non-admin cannot create
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/content/" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"title":"Fail","description":"Should fail","content_type":"movie","duration_minutes":90,"release_date":"2024-01-01T00:00:00"}')
check "Non-admin cannot create content" "$R" "403"

# More content for variety
for i in 1 2 3; do
  curl -s -o /tmp/z_last.json -X POST "$BASE/content/" \
    -H "$AUTH_ADMIN" -H "Content-Type: application/json" \
    -d "{
      \"title\":\"Test Movie $i\",\"description\":\"Test description $i for search\",\"content_type\":\"movie\",
      \"duration_minutes\":$((90+i*10)),\"release_date\":\"202${i}-06-15T00:00:00\",\"rating\":0,
      \"genres\":[\"Action\",\"Comedy\"],\"cast\":[\"Actor $i\"],\"directors\":[\"Director $i\"]
    }"
done

# ── FR-34: Batch Content Ingestion ─────────────────────────────────
header "FR-34 Batch Content Ingestion"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/content/batch" \
  -H "$AUTH_ADMIN" -H "Content-Type: application/json" \
  -d '[
    {"title":"Inception","description":"A thief who steals corporate secrets through dream-sharing","content_type":"movie","duration_minutes":148,"release_date":"2010-07-16T00:00:00","genres":["Sci-Fi","Action"],"cast":["Leonardo DiCaprio"],"directors":["Christopher Nolan"]},
    {"title":"Dunkirk","description":"Allied soldiers are surrounded by enemy on the beach of Dunkirk","content_type":"movie","duration_minutes":106,"release_date":"2017-07-21T00:00:00","genres":["War","Drama"],"cast":["Fionn Whitehead"],"directors":["Christopher Nolan"]}
  ]')
check "Batch create content" "$R" "200"

# ── FR-04: Content Catalog Browsing ────────────────────────────────
header "FR-04 Content Catalog Browsing"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/")
check "List all content" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?content_type=movie")
check "Filter by type (movie)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?genre=Sci-Fi")
check "Filter by genre (Sci-Fi)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?query=batman")
check "Search by keyword (batman)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?actor=McConaughey")
check "Filter by actor" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?director=Nolan")
check "Filter by director" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/$MOVIE1_ID")
check "Get content by ID" "$R" "200"

# ── Semantic Search (ChromaDB) ────────────────────────────────────
header "Semantic Search (ChromaDB)"

# Semantic match — query terms not present verbatim in title/desc/cast.
# "space exploration" should surface Interstellar via vector similarity.
R=$(do_curl -s -o /tmp/z_semantic1.json -w "%{http_code}" "$BASE/content/?query=space%20exploration&min_similarity=0.2")
check "Semantic search (space exploration)" "$R" "200" /tmp/z_semantic1.json
SEMANTIC_HITS=$(cat /tmp/z_semantic1.json | python3 -c "import sys,json; print(len(json.load(sys.stdin)))" 2>/dev/null)
if [ "${SEMANTIC_HITS:-0}" -gt 0 ]; then
    green "Semantic search returned $SEMANTIC_HITS result(s)"
    PASS=$((PASS+1))
else
    red "Semantic search returned 0 results (ChromaDB may not be indexed)"
    FAIL=$((FAIL+1))
fi
TOTAL=$((TOTAL+1))

# Conceptual query — "criminal mastermind" should match Dark Knight/Breaking Bad description.
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?query=criminal%20mastermind&min_similarity=0.2")
check "Semantic search (criminal mastermind)" "$R" "200"

# min_similarity boundary checks
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?query=batman&min_similarity=0.0")
check "Search with min_similarity=0.0 (broadest)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?query=batman&min_similarity=1.0")
check "Search with min_similarity=1.0 (strictest)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?query=batman&min_similarity=1.5")
check "Reject min_similarity > 1.0" "$R" "422"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?query=batman&min_similarity=-0.1")
check "Reject min_similarity < 0.0" "$R" "422"

# ── FR-18: Filter by Release Year ─────────────────────────────────
header "FR-18 Filter by Release Year"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?year=2014")
check "Filter by release year (2014)" "$R" "200"

# ── FR-19: Content Sorting ─────────────────────────────────────────
header "FR-19 Content Sorting"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?sort_by=title&sort_order=asc")
check "Sort by title ascending" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?sort_by=rating&sort_order=desc")
check "Sort by rating descending" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/?sort_by=release_date&sort_order=desc")
check "Sort by release date" "$R" "200"

# ── FR-20: Top Rated Content ──────────────────────────────────────
header "FR-20 Top Rated Content"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/top-rated")
check "Get top rated content" "$R" "200"

# ── FR-21: Recently Added Content ─────────────────────────────────
header "FR-21 Recently Added Content"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/recently-added")
check "Get recently added content" "$R" "200"

# ── FR-22: Genre Listing ──────────────────────────────────────────
header "FR-22 Genre Listing"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/genres")
check "List all genres" "$R" "200"

# ── FR-28: Content Stats ──────────────────────────────────────────
header "FR-28 Content Stats by Type/Genre"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/stats")
check "Content count by type and genre" "$R" "200"

# ── FR-32: Cast/Director Search ───────────────────────────────────
header "FR-32 Cast/Director Search"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/content/cast-search?query=Nolan")
check "Search by cast/director name" "$R" "200"

# ── Content Update / Delete ───────────────────────────────────────
header "Content Update/Delete (Admin)"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X PUT "$BASE/content/$MOVIE1_ID" \
  -H "$AUTH_ADMIN" -H "Content-Type: application/json" \
  -d '{"description":"Updated description for Interstellar"}')
check "Admin updates content" "$R" "200"

# ── FR-11: Content Ratings & Reviews ──────────────────────────────
header "FR-11 Content Ratings & Reviews"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/ratings/$MOVIE1_ID" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"score":5,"review":"Mind-blowing film about space and time!"}')
check "Rate content with review (5 stars)" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/ratings/$MOVIE2_ID" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"score":4.5,"review":"Best superhero movie ever made"}')
check "Rate second content" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/ratings/$SERIES1_ID" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"score":5}')
check "Rate content without review" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/ratings/$MOVIE1_ID" -H "$AUTH_USER")
check "Get rating summary (avg, total, user score)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/ratings/$MOVIE1_ID/reviews")
check "Get content reviews" "$R" "200"

# ── FR-23: User's Rating History ──────────────────────────────────
header "FR-23 User's Rating History"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/ratings/my-ratings" -H "$AUTH_USER")
check "Get my rating history" "$R" "200"

# ── FR-10: Watchlist ──────────────────────────────────────────────
header "FR-10 Watchlist (Save for Later)"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/users/$USER_ID/watchlist/$MOVIE1_ID" -H "$AUTH_USER")
check "Add to watchlist" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/users/$USER_ID/watchlist/$SERIES1_ID" -H "$AUTH_USER")
check "Add second item to watchlist" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/users/$USER_ID/watchlist" -H "$AUTH_USER")
check "Get watchlist" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X DELETE "$BASE/users/$USER_ID/watchlist/$SERIES1_ID" -H "$AUTH_USER")
check "Remove from watchlist" "$R" "200"

# ── FR-06: Playback & Watch Tracking (Cassandra) ──────────────────
header "FR-06 Playback & Watch Tracking"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/watch/$MOVIE1_ID/event" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"event_type":"play","position_seconds":0}')
check "Record playback event (play)" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/watch/$MOVIE1_ID/event" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"event_type":"pause","position_seconds":3600}')
check "Record playback event (pause)" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/watch/$MOVIE1_ID/event" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"event_type":"resume","position_seconds":3600}')
check "Record playback event (resume)" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/watch/$MOVIE2_ID/event" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"event_type":"play","position_seconds":0}')
check "Record play event on second movie" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/watch/$MOVIE2_ID/event" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"event_type":"complete","position_seconds":9120}')
check "Record complete event" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/watch/$MOVIE1_ID/events" -H "$AUTH_USER")
check "Get playback events" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/watch/$MOVIE1_ID/resume" -H "$AUTH_USER")
check "Get resume position" "$R" "200"

# ── FR-07: Watch History ──────────────────────────────────────────
header "FR-07 Watch History"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/watch/history/me" -H "$AUTH_USER")
check "Get my watch history" "$R" "200"

# ── FR-08: Personalized Homepage ──────────────────────────────────
header "FR-08 Personalized Homepage"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/watch/continue-watching" -H "$AUTH_USER")
check "Continue watching (incomplete titles)" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/recommendations/homepage" -H "$AUTH_USER")
check "Personalized homepage (all rows)" "$R" "200"

# ── FR-09: Graph-Based Recommendations ────────────────────────────
header "FR-09 Graph-Based Recommendations"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/recommendations/for-you" -H "$AUTH_USER")
check "Get recommendations for you" "$R" "200"

# ── FR-14: Content Similarity ─────────────────────────────────────
header "FR-14 Content Similarity (More Like This)"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/recommendations/similar/$MOVIE1_ID")
check "Get similar content (Interstellar)" "$R" "200"

# ── FR-09 extra: User Graph ───────────────────────────────────────
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/recommendations/user-graph" -H "$AUTH_USER")
check "Get user graph data" "$R" "200"

# ── FR-12: Multi-Profile Support ──────────────────────────────────
header "FR-12 Multi-Profile Support"

R=$(do_curl -s -o /tmp/z_profile1.json -w "%{http_code}" -X POST "$BASE/users/$USER_ID/profiles" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"name":"Kids Profile","maturity_level":"kids"}')
check "Create profile (Kids)" "$R" "201" /tmp/z_profile1.json
PROFILE_ID=$(cat /tmp/z_profile1.json | python3 -c "import sys,json; print(json.load(sys.stdin)['profile_id'])" 2>/dev/null)

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X POST "$BASE/users/$USER_ID/profiles" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"name":"Teen Profile","maturity_level":"teen"}')
check "Create profile (Teen)" "$R" "201"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/users/$USER_ID/profiles" -H "$AUTH_USER")
check "List profiles" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X PUT "$BASE/users/$USER_ID/profiles/$PROFILE_ID" \
  -H "$AUTH_USER" -H "Content-Type: application/json" \
  -d '{"name":"Updated Kids"}')
check "Update profile" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X DELETE "$BASE/users/$USER_ID/profiles/$PROFILE_ID" -H "$AUTH_USER")
check "Delete profile" "$R" "204"

# ── FR-24: Admin — List Users ─────────────────────────────────────
header "FR-24 Admin List Users"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/users/" -H "$AUTH_ADMIN")
check "Admin lists all users" "$R" "200"

# Non-admin cannot list
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/users/" -H "$AUTH_USER")
check "Non-admin cannot list users" "$R" "403"

# ── FR-33: Search Users ──────────────────────────────────────────
header "FR-33 Search Users"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/users/?search=normal" -H "$AUTH_ADMIN")
check "Search users by username" "$R" "200"

# ── FR-25: Admin — Deactivate User ────────────────────────────────
header "FR-25 Admin Activate/Deactivate User"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X PATCH "$BASE/users/$USER_ID/activate?active=false" -H "$AUTH_ADMIN")
check "Admin deactivates user" "$R" "200"

R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X PATCH "$BASE/users/$USER_ID/activate?active=true" -H "$AUTH_ADMIN")
check "Admin reactivates user" "$R" "200"

# ── FR-15: Platform Analytics (Admin) ─────────────────────────────
header "FR-15 Platform Analytics"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/admin/analytics" -H "$AUTH_ADMIN")
check "Admin analytics dashboard" "$R" "200"

# ── FR-29: User Growth ────────────────────────────────────────────
header "FR-29 User Activity Summary"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/admin/user-growth?days=30" -H "$AUTH_ADMIN")
check "User growth over 30 days" "$R" "200"

# ── FR-30: Trending Content ──────────────────────────────────────
header "FR-30 Trending Content"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/admin/trending?days=7" -H "$AUTH_ADMIN")
check "Trending content (last 7 days)" "$R" "200"

# ── FR-35: Database Health Check ─────────────────────────────────
header "FR-35 Database Health Check"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" "$BASE/admin/health" -H "$AUTH_ADMIN")
check "All-database health check" "$R" "200"

# ── FR-31: Delete Rating ─────────────────────────────────────────
header "FR-31 Delete Rating"
R=$(do_curl -s -o /tmp/z_last.json -w "%{http_code}" -X DELETE "$BASE/ratings/$SERIES1_ID" -H "$AUTH_USER")
check "Delete a rating" "$R" "204"

# ── Cleanup: Delete content (admin) ───────────────────────────────
header "Cleanup"
# We don't delete everything so the DB has data to inspect manually

# ════════════════════════════════════════════════════════════════════
# Summary
# ════════════════════════════════════════════════════════════════════
echo ""
echo "════════════════════════════════════════════════════════"
printf "  RESULTS: \033[32m%d passed\033[0m / \033[31m%d failed\033[0m / %d total\n" "$PASS" "$FAIL" "$TOTAL"
echo "════════════════════════════════════════════════════════"

if [ "$FAIL" -gt 0 ]; then
    exit 1
fi
